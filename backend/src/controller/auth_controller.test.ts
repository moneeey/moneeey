...
describe("auth_controller", () => {
  ...
  describe("start", () => {
    it("success new user", async () => {
      ...
      expect(await auth.start("moneeey@baroni.tech")).toEqual({
        auth_code:
          "hashed:-123450004-moneeey@baroni.tech_auth_UUIDUUID-dcf7-6969-a608-420123450003",
        ...
        [
          {
            ...
            text: "Please click the following link to complete your registration: http://local.moneeey.io/?auth_code=hashed:-123450004-moneeey@baroni.tech_auth_UUIDUUID-dcf7-6969-a608-420123450003&confirm_code=hashed:-123450006-hashed:-123450004-moneeey@baroni.tech_auth_UUIDUUID-dcf7-6969-a608-420123450003moneeey@baroni.tech_auth_UUIDUUID-dcf7-6969-a608-420123450005&email=moneeey@baroni.tech",
      ...
      expect(logger.history).toEqual([
        {
          debug: [
            "get_or_create_user retrieve",
            { email: "moneeey@baroni.tech" },
          ],
        },
        {
          debug: [
            "get_or_create_user created default",
            { email: "moneeey@baroni.tech" },
          ],
        },
    ...
    it("success existing user", async () => {
      ...
      expect(await auth.start("moneeey@baroni.tech")).toEqual({
        auth_code:
          "hashed:-123450002-moneeey@baroni.tech_auth_UUIDUUID-dcf7-6969-a608-420123450001",
        email: "moneeey@baroni.tech",
        success: true,
      });
      expect(mainDb.history).toEqual([
  ...
  describe("check", () => {
    it("success", async () => {
      ...
      expect(
        await auth.check("moneeey@baroni.tech", "correct_auth_code")
      ).toEqual({
        success: true,
      });
      expect(mainDb.history).toEqual([
    ...
    it("fails due to invalid auth code", async () => {
      ...
      expect(
        await auth.check("moneeey@baroni.tech", "incorrect_auth_code")
      ).toEqual({
        success: false,
      });
      expect(mainDb.history).toEqual([
    ...
    it("fails due to expired auth", async () => {
      ...
      expect(logger.history).toEqual([
        ...
        },
        {
          error: [
            "complete - error_code",
            { email: "moneeey@baroni.tech", error: "code_expired" },
          ],
        },
      ]);
    });
    it("fails due to invalid auth_code", async () => {
      ...
      expect(logger.history).toEqual([
        ...
        },
        {
          error: [
            "complete - error_code",
            { email: "moneeey@baroni.tech", error: "auth_code" },
          ],
        },
      ]);
    });
    it("fails due to invalid confirm_code", async () => {
      ...
      expect(logger.history).toEqual([
        ...
        },
        {
          error: [
            "complete - error_code",
            { email: "moneeey@baroni.tech", error: "confirm_code" },
          ],
        },
      ]);
    });
  ...
  describe("logout", () => {
    it("success", async () => {
      ...
      expect(
        await auth.logout("moneeey@baroni.tech", "correct_auth_code")
      ).toEqual({
        success: true,
      });
      expect(mainDb.history).toEqual([
