...
import {
  ...
  APP_URL,
  HASH_PREFIX,
  MAX_AUTHENTICATION_LIFE_SECONDS,
  MAX_AUTHENTICATION_SECONDS,
} from "../core/config";
...
export default class AuthController extends DatabaseController {
  ...
  async authenticate(email: string, auth_code: string) {
    const mainDb = this.connect_main_db();
    const user = await this.get_or_create_user(mainDb, email);
    const auth = user.auth.find(
      (auth) =>
        auth.auth_code === auth_code &&
        auth.confirmed &&
        auth.updated > tick() - MAX_AUTHENTICATION_LIFE_SECONDS
    );
    if (auth) {
      auth.updated = tick();
      return user;
  ...
  async complete(email: string, auth_code: string, confirm_code: string) {
    ...
    if (auth.created < tick() - MAX_AUTHENTICATION_SECONDS)
