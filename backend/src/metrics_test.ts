...
Deno.test(function countersGaugesHistogramsRender() {
	...
	metrics.dbDuration.observe(0.003, { op: "push", engine: "sqlite" });
	...
	has('moneeey_db_op_duration_seconds_count{engine="sqlite",op="push"} 1');
	has(
		'moneeey_db_op_duration_seconds_bucket{engine="sqlite",le="+Inf",op="push"} 1',
	);
});
