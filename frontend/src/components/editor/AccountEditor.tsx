...
interface AccountEditorBaseProps<EntityType> extends EditorProps<EntityType, TAccountUUID, TAccountUUID> {
  accounts: IAccount[];
  value: TAccountUUID;
  rev?: string;
  entity?: EntityType;
...
const AccountEditorBase = observer(<EntityType,>(props: AccountEditorBaseProps<EntityType>) => {
  ...
  const options = compact([
    !isEmpty(adding) && { label: Messages.settings.create_entity(adding), value: addPrefix + adding }
    ,...map(props.accounts, (account) => ({ label: account.name, value: account.account_uuid }))
  ])
  return (
      ...
      <BaseSelectEditor
        {...{
          ...
          ComposedProps: (onChange: (value?: string, editorValue?: string, additional?: Partial<EntityType>) => void) => ({
            showSearch: true,
            filterOption: (inputValue: string, option?: typeof options[number]) => option?.label.toLowerCase().includes(inputValue.toLowerCase()),
            ...
            onSelect: (value?: string) => {
              if (value?.startsWith(addPrefix)) {
                ...
                if (props.entity && isTransaction(props.entity)) {
                  const transaction_currencies = [props.entity.from_account, props.entity.to_account]
                    .map(account_uuid => accounts.byUuid(account_uuid)?.currency_uuid)
...
export const AccountSelector = ({ account, accounts, onSelect }: AccountSelectorProps) => (
  <AccountEditorBase
    accounts={accounts}
    value={account}
    entityId={account}
    field={{ editor: EditorType.ACCOUNT, field: 'selectorz', index: 0, title: '', }}
    onUpdate={(value) => onSelect(value)}
    store={{ byUuid: () => ({ selectorz: account } as unknown as IAccount) } as unknown as MappedStore<IAccount>} />
)
...
export const AccountEditor = observer(<EntityType,>(props: EditorProps<EntityType, TAccountUUID, TAccountUUID>) => {
  ...
  return (
    <AccountEditorBase {...{ ...props, accounts: accounts.all, value, rev: entity?._rev || '' }} />
  )
})
