...
export default function <TEntity>({
  ...
  return {
    render: observer(({ entity, commit, field, isError }: FieldRenderProps<TEntity>) => {
      ...
      const tags = accounts.accountTags(read(entity)).filter((t) => t !== currentName);
      const Tags = field.title === Messages.transactions.from_account ? TagsFrom : TagsTo;
          ...
          onCreate={(name: string) => {
            ...
            if (isTransaction(withEntityType)) {
              account.currency_uuid =
                compact(
                  [withEntityType.from_account, withEntityType.to_account].map(
    ...
    sorter: (a: TEntity, b: TEntity, asc: boolean): number =>
      asc ? readName(a).localeCompare(readName(b)) : readName(b).localeCompare(readName(a)),
  };
}
