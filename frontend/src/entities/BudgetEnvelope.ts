...
import { TDate, formatDate, parseDate, startOfMonthOffset } from '../utils/Date';
...
export class BudgetEnvelopeStore extends MappedStore<BudgetEnvelope> {
  ...
  getEnvelope(entity: IBudget, starting: TDate) {
    ...
    const allocated = entity.envelopes.find((envelop) => envelop.starting === starting)?.allocated || 0;
  ...
  merge(item: BudgetEnvelope, options?: { setUpdated: boolean }): void {
    this.moneeeyStore.budget.setAllocation(item.budget, item.starting, item.allocated);
  ...
  getRemaining(envelope: BudgetEnvelope): number {
    ...
    const hasPreviousTransaction =
      startOfMonthOffset(this.moneeeyStore.transactions.oldest_dt, -1).getTime() < previousEnvelopeStarting.getTime();
    ...
    const usages = await asyncProcess(
      ...
      (chunk, state, percentage) => {
        ...
        chunk.forEach((transaction) => {
          ...
          const budgets = this.moneeeyStore.budget.findBudgetsFor(allTransactionTags);
      ...
      { state: {} as Record<string, { usage: number; envelope: BudgetEnvelope }>, chunkSize: 200, chunkThrottle: 20 }
    );
    values(usages).map(({ envelope, usage }) => this.updateVirtualEnvelopeUsage(envelope, usage));
    this.updateRemainings();
  }, 500);
}
