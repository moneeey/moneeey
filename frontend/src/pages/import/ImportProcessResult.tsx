  ...
  return (
    <>
      {map(result.errors, err => (
        <p><Typography.Text code key={err.description} type='danger' title={err.data}>
          {err.description}
        </Typography.Text></p>))}
      <Table
        ...
        columns={[
          ...
          ...col, render: (value: any, row: ITransaction) => {
            let mode = 'New'
            let title = mode
            if (col.dataIndex) {
              ...
              if (original) {
                ...
                if (!isEmpty(originalValue) || isNumber(originalValue)) {
                  const format = (value: string) => col.dataIndex.indexOf('_account') > 0 ? moneeeyStore.accounts.nameForUuid(value) : '' + value
                  const changed = format(originalValue) !== format(value)
                  mode = changed ? 'Updated' : 'Unchanged'
                  title = changed ? 'Changed from \n' + format(originalValue) + '\nto\n' + format(value) : 'Unchanged'
                }
              }
            }
