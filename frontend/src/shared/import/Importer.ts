...
class Importer {
  ...
  get tokenMap() {
    ...
    const tokenMap = this.moneeeyStore.transactions.all.reduce(
      (accum, transaction) => {
        const tokens = uniq(
          compact(
            flatten([
  ...
  findAccountsForTokens(
    ...
    tokenMap: Map<ITransaction, string[]>,
    _tokens: string[]
  ) {
    const MAX_MATCH = 100
    ...
    const matchingTransactions = reduce(
      ...
      (results, [transaction, transactionTokens]) => {
        if (results.length < MAX_MATCH) {
          const common: string[] = transactionTokens.filter((tok) =>
            includes(tokens, tok)
          )
          if (common.length > 0) {
    ...
    const nonReferenceAccount = filter(
      ...
      filterNonReferenceAccount
    )
    const mostMatchedFirst = uniq(
      reverse(
        sortBy(values(groupBy(nonReferenceAccount, identity)), (v) => v.length)
      )
    )
    const mostMatched = compact(mostMatchedFirst.map(head)).map(
      (account_uuid) => ({
        account_uuid,
    ...
    console.log('Importer.findAccountsForTokens', {
      referenceAccount,
      tokenMap,
      tokens,
      mostMatchedFirst,
