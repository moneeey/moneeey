...
import { AUTH_MAX_TIME, Management } from './management';

dotenv.config({ path: './sample.env' });

const DEFAULT_DB_NAME = 'moneeey';
  ...
  return {
    connect<T>(dbName: string): PouchDB.Database<T> {
      if (!connectedDBs[dbName]) {
        connectedDBs[dbName] = makeDb(dbName);
      }
...
describe('management server', () => {
  ...
  const emailHash =
    'fe505a016c5edc723fc838853c0dd47379f6712e6d72ba9bb655dac4f9c6832254f95f8d93f38fc85e9522619fb2f2c0968d19406c8bcedc32fe8dfcfadd7ce0';
  ...
  beforeEach(() => {
    ...
    jest.spyOn(Math, 'random').mockImplementation(() => 0.42);
  ...
  describe('hash', () => {
    it('hash email', () => {
      expect(env.AUTH_HASH_PREFIX).toEqual('somethingveryrandom');
  ...
  function sendEmailLog(confirmCode: string = '') {
    return [
      ...
      {
        content:
          'Please click the following link to complete your registration http://frontend.moneeey.local:3000/auth/?code=' +
          confirmCode +
          '&email=fernando@baroni.tech',
        subject: `${APP_DESC} registration`,
  ...
  describe('auth_start', () => {
    it('success - creates new user', async () => {
      ...
      expect(await management.auth_start(email).toPromise()).toEqual({
        login: '0022008b603cbc4e203203a00e32e3ba',
        status: 'started'
      });
      ...
      expect(consoleMock.history).toEqual([{ log: sendEmailLog('6bd1ac10c31b02de0a5e9c28528be720') }]);
    });
    ...
    it('success - existing user', async () => {
      ...
      expect(await management.auth_start(email).toPromise()).toEqual({
        login: 'f08e423c80f4f94abafcd941abb98f90',
        status: 'started'
      });
      ...
      expect(consoleMock.history).toEqual([{ log: sendEmailLog('8313f4aa0ae393dbe9b005b4841a12b4') }]);
    });
  });
  ...
  describe('auth_complete', () => {
    it('success - creates new session', async () => {
      ...
      expect(await management.auth_complete(email, AUTH_1.confirm_code).toPromise()).toEqual({ status: 'confirmed' });
    ...
    it('confirmation_code not found', async () => {
      dbMock.spy.mockResolvedValueOnce({
        _id: 'user_fe505a016c5edc723fc838853c0dd47379f6712e6d72ba9bb655dac4f9c6832254f95f8d93f38fc85e9522619fb2f2c0968d19406c8bcedc32fe8dfcfadd7ce0',
        _rev: 'created',
        auth: [AUTH_1, AUTH_2],
        created: 1634439600001,
      ...
      expect(await management.auth_complete(email, 'idontexist').toPromise()).toEqual({ status: 'error' });
      ...
      expect(consoleMock.history).toEqual([
        {
          error: ['auth_complete', { error: 'confirmation_code_not_found' }]
        }
      ]);
    });
    ...
    it('expired', async () => {
      ...
      expect(await management.auth_complete(email, AUTH_1.confirm_code).toPromise()).toEqual({ status: 'error' });
      ...
      expect(consoleMock.history).toEqual([
        {
          error: ['auth_complete', { error: 'confirmation_code_expired' }]
        }
      ]);
    });
  });
